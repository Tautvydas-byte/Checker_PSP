...
public class CheckersBehavior {
    ...
    public ArrayList<Position> getPossibleMoves(Checker selectedChecker, boolean playerTurn, ArrayList<Checker> checkers, ArrayList<Checker> checkersOpponent, GameField gameField) {
        ...
        Position selectedCheckerPosition = selectedChecker.getPosition();
            ...
            if(gameField.isLeftLimitSide(selectedCheckerPosition))//parenka tinkama pozicija
            {
                if(!thereIsCheckerInPosition(selectedCheckerPosition.rightUp(), checkers) && playerTurn)//mano
                {
                    possiblePositions.add( new Position(selectedCheckerPosition.rightUp()));
                }
                if(!thereIsCheckerInPosition(selectedCheckerPosition.rightDown(), checkers) && !playerTurn)//oponento
                {
                    possiblePositions.add( new Position(selectedCheckerPosition.rightDown()));
                }
            }

            else if (gameField.isRightLimitSide(selectedCheckerPosition))
            {
            ...
            else if (gameField.isLowerLimitSide(selectedCheckerPosition))
            {
    ...
    private boolean thereIsCheckerInPosition(Position position, ArrayList<Checker> checkers)//ar position priklauso kazkuriam is checkeriu
    {
